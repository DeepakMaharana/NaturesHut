...
const SignIn: React.FC = () => {
  ...
  const { loading, error, postData } = usePost<any>();
  ...
  const handelSignIn = async (e) => {
    ...
    let signInSchema = object({
      email: string().email("Invalid email address").matches(
      ...
      password: string()
        ...
        .matches(
          /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$/,
          "Password must contain at least one symbol, number, smallcase letter and uppercase letter"
    ...
    try {
      const formValidation = await signInSchema.validate(obj, {abortEarly: false});
      ...
      try {
        const response = await postData('/signin',obj);
        if(response)
        {
  ...
  return (
    <>
      ...
      <div className="w-full sm:w-[80%] my-10 mx-auto rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
